...
__global__
void Fold2d_Kernel( Flow::NArrayDevice* arr, Flow::NArrayDevice* result, int batchSize,
    int channels, int kernelHeight, int kernelWidth, int outHeight, int outWidth )
{
...
NARRAY Flow::Fold2d( NARRAY arr, vector<int> outShape, vector<int> kernel )
{
    ...
    Fold2d_Kernel<<< numBlocks, threadsPerBlock >>>( arr->DeviceStruct, result->DeviceStruct,
        batchSize, channels, kernelHeight, kernelWidth, outHeight, outWidth );
    CUDA_DeviceSynchronize();
    return result;
}

__global__
void BackwardFold2d_Kernel( Flow::NArrayDevice* arr, Flow::NArrayDevice* grad,
...
void Flow::NArray::BackwardFold2d()
{
    ...
    int blocksY = ( outHeight + threadsPerBlock.y - 1 ) / threadsPerBlock.y;
    dim3 numBlocks( blocksX, blocksY );
    BackwardFold2d_Kernel<<< numBlocks, threadsPerBlock >>>( DeviceStruct, Gradient->DeviceStruct,
