...
__global__
void Mul_Kernel( Flow::NArrayDevice* arr1, Flow::NArrayDevice* arr2, Flow::NArrayDevice* result,
    int n )
{
...
NARRAY Flow::Mul( NARRAY arr1, NARRAY arr2 )
{
    ...
    NARRAY result = make_shared<NArray>( resultShape, vector<NARRAY>({ arr1B, arr2B }),
        NArray::Operation::MUL );
    int n = SizeFromShape(resultShape);
    Mul_Kernel<<< BLOCKS(n), TPB >>>( arr1B->GetDeviceStruct(), arr2B->GetDeviceStruct(),
...
__global__
void BackwardMul_Kernel( Flow::NArrayDevice* grad, Flow::NArrayDevice* operand1,
    Flow::NArrayDevice* operandGrad1, Flow::NArrayDevice* operand2,
    Flow::NArrayDevice* operandGrad2, int n )
{
    ...
    Flow::AtomicAdd_Device( &operandGrad1->Data[operandGradIndex1],
        operand2->Data[operandIndex2] * grad->Data[gradIndex] );
    Flow::AtomicAdd_Device( &operandGrad2->Data[operandGradIndex2],
        operand1->Data[operandIndex1] * grad->Data[gradIndex] );
}

void Flow::NArray::BackwardMul()
{
    int n = SizeFromShape(Gradient->Shape);
    BackwardMul_Kernel<<< BLOCKS(n), TPB >>>( Gradient->DeviceStruct, Operands[0]->DeviceStruct,
        Operands[0]->Gradient->DeviceStruct, Operands[1]->DeviceStruct,
        Operands[1]->Gradient->DeviceStruct, n );
    CUDA_DeviceSynchronize();
}