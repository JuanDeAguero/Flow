...
__global__
void Unfold2d_Kernel( Flow::NArrayDevice* arr, Flow::NArrayDevice* result, int batchSize,
    int channels, int kernelHeight, int kernelWidth, int strideHeight, int strideWidth,
...
NARRAY Flow::Unfold2d( NARRAY arr, vector<int> kernel, vector<int> stride )
{
    ...
    vector<int> resultShape = { batchSize, channels * kernelHeight * kernelWidth, outHeight *
        outWidth };
    NARRAY result = make_shared<NArray>( resultShape, vector<NARRAY>({ arr }),
        NArray::Operation::UNFOLD2D );
    ...
    Unfold2d_Kernel<<< numBlocks, threadsPerBlock >>>( arr->DeviceStruct, result->DeviceStruct,
        batchSize, channels, kernelHeight, kernelWidth, strideHeight, strideWidth, outHeight,
        outWidth );
...
__global__
void BackwardUnfold2d_Kernel( Flow::NArrayDevice* arr, Flow::NArrayDevice* grad,
    Flow::NArrayDevice* operand, Flow::NArrayDevice* operandGrad, int batchSize, int channels,
...
void Flow::NArray::BackwardUnfold2d()
{
    ...
    int blocksY = ( outHeight + threadsPerBlock.y - 1 ) / threadsPerBlock.y;
    dim3 numBlocks( blocksX, blocksY );
    BackwardUnfold2d_Kernel<<< numBlocks, threadsPerBlock >>>( DeviceStruct, Gradient->DeviceStruct,
        Operands[0]->DeviceStruct, Operands[0]->Gradient->DeviceStruct, batchSize, channels,
