...
__global__
void BMM_Kernel( Flow::NArrayDevice* arr1, Flow::NArrayDevice* arr2, Flow::NArrayDevice* result,
    int arr1Rows, int arr1Cols, int arr2Cols, int batchSize )
{
...
NARRAY Flow::BMM( NARRAY arr1, NARRAY arr2 )
{
    ...
    NARRAY result = make_shared<NArray>( resultShape, vector<NARRAY>({ arr1, arr2 }),
        NArray::Operation::BMM );
    dim3 dimGrid( ceil( arr2Cols / float(TILE_SIZE) ), ceil( arr1Rows / float(TILE_SIZE) ),
        batchSize );
    dim3 dimBlock( TILE_SIZE, TILE_SIZE, 1 );
    BMM_Kernel<<< dimGrid, dimBlock >>>( arr1->GetDeviceStruct(), arr2->GetDeviceStruct(),
        result->GetDeviceStruct(), arr1Rows, arr1Cols, arr2Cols, batchSize );
