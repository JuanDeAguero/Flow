...
__global__
void Gather_Kernel( float* arr, int* arrShape, int arrShapeSize, int dim, float* index, int indexSize, int* indexShape, int indexShapeSize, float* result, int resultSize )
{
    int i = blockIdx.x;
...
Flow::NArrayCore* Flow::Gather( NArrayCore* arr, int dim, NArrayCore* index )
{
    ...
    Gather_Kernel<<< n, 1 >>>( arr->GetData(), arrShape_d, arr->GetShape().size(), dim, index->GetData(), SizeFromShape(index->GetShape()), indexShape_d, index->GetShape().size(), result_d, n );
...
__global__
void BackwardGather_Kernel( float* gradient, int* operandShape, int operandShapeSize, float* operandGradient, int operandGradientSize, int dim, float* index, int indexSize, int* indexShape, int indexShapeSize )
{
    int i = blockIdx.x;
    if (i >= indexSize) return;
    int multiIndex[MAX_DIMS];
...
void Flow::NArrayCore::BackwardGather()
{
    ...
    BackwardGather_Kernel<<< n, 1 >>>( Gradient->GetData(), operandShape_d, Operands[0]->GetShape().size(), Operands[0]->GetGradient()->GetData(), SizeFromShape(Operands[0]->GetGradient()->GetShape()), GatherDim, GatherIndex->GetData(), SizeFromShape(GatherIndex->GetShape()), indexShape_d, GatherIndex->GetShape().size() );
    cudaDeviceSynchronize();
