...
__global__
void Gather_Kernel( Flow::NArrayDevice* arr, Flow::NArrayDevice* result, int resultSize, int dim,
...
{
    NARRAY result = make_shared<NArray>( index->Shape, vector<NARRAY>({ arr }),
        NArray::Operation::GATHER );
    result->GatherDim = dim;
    result->GatherIndex = index;
    int n = SizeFromShape(result->Shape);
    Gather_Kernel<<< BLOCKS(n), TPB >>>( arr->DeviceStruct, result->DeviceStruct,
        SizeFromShape(arr->Shape), dim, index->DeviceStruct, n );
    CUDA_DeviceSynchronize();
    return result;
}

__global__
void BackwardGather_Kernel( Flow::NArrayDevice* grad, Flow::NArrayDevice* operand,
    Flow::NArrayDevice* operandGrad, int operandGradSize, int dim, Flow::NArrayDevice* index,
    int n )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if ( i >= n ) return;
...
void Flow::NArray::BackwardGather()
{
    ...
    BackwardGather_Kernel<<< BLOCKS(n), TPB >>>( Gradient->DeviceStruct, Operands[0]->DeviceStruct,
        Operands[0]->Gradient->DeviceStruct, SizeFromShape(Operands[0]->Gradient->Shape), GatherDim,
        GatherIndex->DeviceStruct, n );
    CUDA_DeviceSynchronize();
}