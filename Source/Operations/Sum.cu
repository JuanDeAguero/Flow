...
Flow::NArrayCore* Flow::Sum( NArrayCore* arr, int dim )
{
...
__global__
void BackwardSum_Kernel( float* arr, int* shape, int shapeSize, float* gradient, float* operand, int* operandShape, int operandShapeSize, float* operandGradient, int dim )
{
    int i = blockIdx.x;
    int j = blockIdx.y;
    int multiIndex[10];
