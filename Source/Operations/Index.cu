...
__global__
void Index_Kernel( float* arr, int* arrShape, int arrShapeSize, int dim, float* index, float* result, int* resultShape, int resultShapeSize )
{
    int i = blockIdx.x;
...
Flow::NArrayCore* Flow::Index( NArrayCore* arr, int dim, NArrayCore* index )
{
...
__global__
void BackwardIndex_Kernel( int* shape, int shapeSize, float* gradient, int* operandShape, int operandShapeSize, float* operandGradient, int dim, float* index )
{
    int i = blockIdx.x;
    int multiIndex[MAX_DIMS];
