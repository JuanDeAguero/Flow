...
Flow::Optimizer::Optimizer(vector<NARRAY> arrays, float learningRate, float epsilon, float weightDecay)
...
void Flow::Optimizer::ZeroGrad() {
    for (NARRAY arr : Arrays) {
        NARRAY gradient = arr->GetGradient();
        if (gradient) gradient->Reset(0.0f);
    }
}
