...
NARRAY Flow::Prod(NARRAY arr, int dim) {
    ...
    resultShape[dim] = 1;
    NARRAY result = make_shared<NArray>(resultShape, vector<NARRAY>({ arr }), NArray::Operation::PROD);
    result->ProdDim = dim;
    result->Reset(1.0f);
...
__global__
void BackwardProd_Kernel(Flow::NArrayDevice* arr, Flow::NArrayDevice* grad, Flow::NArrayDevice* operand, Flow::NArrayDevice* operandGrad, int dim, int n1, int n2) {
    int i = blockIdx.x;
    int j = blockIdx.y;
    if (i >= n1 || j >= n2) return;
    int multiIndex[MAX_DIMS];
    ...
    Flow::AtomicAdd_Device(&operandGrad->Data[operandGradIndex], grad->Data[gradIndex] * prodWithoutCurrent);
}

void Flow::NArray::BackwardProd() {
    int n1 = SizeFromShape(Shape);
    int n2 = Operands[0]->Shape[ProdDim];
    BackwardProd_Kernel<<<dim3(n1, n2), 1>>>(DeviceStruct, Gradient->DeviceStruct, Operands[0]->DeviceStruct, Operands[0]->Gradient->DeviceStruct, ProdDim, n1, n2);
    CUDA_DeviceSynchronize();
}