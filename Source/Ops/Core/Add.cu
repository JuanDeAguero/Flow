...
__global__
void Add_Kernel(Flow::NArrayDevice* arr1, Flow::NArrayDevice* arr2, Flow::NArrayDevice* result, int n) {
...
NARRAY Flow::Add(NARRAY arr1, NARRAY arr2) {
    ...
    NARRAY result = make_shared<NArray>(resultShape, vector<NARRAY>({ arr1B, arr2B }), NArray::Operation::ADD);
    int n = SizeFromShape(resultShape);
    Add_Kernel<<<BLOCKS(n), TPB>>>(arr1B->GetDeviceStruct(), arr2B->GetDeviceStruct(), result->GetDeviceStruct(), n);
    CUDA_DeviceSynchronize();
    return result;
}

__global__
void BackwardAdd_Kernel(Flow::NArrayDevice* grad, Flow::NArrayDevice* operandGrad1, Flow::NArrayDevice* operandGrad2, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
...
void Flow::NArray::BackwardAdd() {
    ...
    BackwardAdd_Kernel<<<BLOCKS(n), TPB>>>(Gradient->DeviceStruct, Operands[0]->Gradient->DeviceStruct, Operands[1]->Gradient->DeviceStruct, n);
    CUDA_DeviceSynchronize();
}