...
NARRAY Flow::Squeeze(NARRAY arr, int dim) {
    ...
    NARRAY result = make_shared<NArray>(resultShape, Flow::StrideFromShape(resultShape), arr->GetOffset(), FindMetaParent(arr), vector<NARRAY>{arr}, NArray::Operation::SQUEEZE);
