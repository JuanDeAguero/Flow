...
NARRAY Flow::Sum(NARRAY arr, int dim) {
    ...
    resultShape[dim] = 1;
    NARRAY result = make_shared<NArray>(resultShape, vector<NARRAY>({ arr }), NArray::Operation::SUM);
    result->SumDim = dim;
...
__global__
void BackwardSum_Kernel(Flow::NArrayDevice* arr, Flow::NArrayDevice* grad, Flow::NArrayDevice* operand, Flow::NArrayDevice* operandGrad, int dim, int n1, int n2) {
    int i = blockIdx.x;
    int j = blockIdx.y;
    if (i >= n1 || j >= n2) return;
    int multiIndex[MAX_DIMS];
...
void Flow::NArray::BackwardSum() {
    ...
    BackwardSum_Kernel<<<dim3(n1, n2), 1>>>(DeviceStruct, Gradient->DeviceStruct, Operands[0]->DeviceStruct, Operands[0]->Gradient->DeviceStruct, SumDim, n1, n2);
    CUDA_DeviceSynchronize();
}