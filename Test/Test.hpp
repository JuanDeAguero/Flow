...
static bool Test( int num, int& numPassed,
    ...
    vector<int> intParams, vector<vector<int>> intVecParams, vector<Flow::NArray> arrParams,
    ...
    vector<float> expectedGradData2, vector<int> expectedGradShape2 )
{
        ...
        Flow::Equals( expectedGradData1, arr1.GetGradient().Get(), 0.01f ) && expectedGradShape1 == arr1.GetGradient().GetShape() )
    {
        if (binaryOp)
        {
            if ( Flow::Equals( expectedGradData2, arr2.GetGradient().Get(), 0.01f ) && expectedGradShape2 == arr2.GetGradient().GetShape() )
                passed = true;
        }
        else passed = true;
    }
