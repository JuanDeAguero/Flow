    ...
    vector<float> expectedGradData2, vector<int> expectedGradShape2 )
{
    ...
    switch (op)
    {
        case Flow::NArrayCore::Operation::ADD: result = Flow::Add( arr1, arr2 ); break;
        case Flow::NArrayCore::Operation::MUL: result = Flow::Mul( arr1, arr2 ); break;
    }
    ...
    {
        Flow::Print("Test_Add_" + to_string(num) + " PASSED");
        numPassed++;
        return true;
    }
    else
    {
        Flow::Print("Test_Add_" + to_string(num) + " FAILED");
        return false;
    }
}